...
function makeSvg(size) {
  ...
  <svg x="${xOff}" y="${yOff}" width="${inner}" height="${scaledH}" viewBox="330 240 ${markW} ${markH}">
    <path fill="rgba(255,255,255,0.97)" d="${LOGO_PATH}"/>
  </svg>
</svg>`;
}

// Generate PNGs at both sizes
for (const size of [192, 512]) {
  const svg = makeSvg(size);
  // Also save SVG for inspection
  if (size === 512) writeFileSync("public/icon-512.svg", svg);

  await sharp(Buffer.from(svg)).png().toFile(`public/icon-${size}.png`);
  console.log(`Generated icon-${size}.png`);
}

// Generate favicon SVG (same design but optimized for small sizes)
const faviconSvg = makeSvg(32);
writeFileSync("public/favicon.svg", makeSvg(512)); // SVG scales, use 512 base
