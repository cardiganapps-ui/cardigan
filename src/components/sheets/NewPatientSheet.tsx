  ...
  return (
    <SheetOverlay exiting={exiting} onClose={submitting ? undefined : animatedClose}>
      ...
      {/* Flex column: header at top, scroll body (flex 1) in the
          ...
          bulletproof option. */}
      <div ref={setPanel} className={`sheet-panel ${exiting ? "sheet-panel--exit" : ""}`} role="dialog" aria-modal="true" aria-label={isPotentialMode ? t("patients.newPotential") : t("patients.newPatient")} {...panelHandlers} style={{ maxHeight:"min(92lvh, calc(100lvh - var(--sat) - 16px))", position:"relative", display:"flex", flexDirection:"column", overflow:"hidden" }}>
        {submitting && (
