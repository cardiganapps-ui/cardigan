...
describe("collapseGroupOccurrences", () => {
  ...
  it("collapses N member rows of one occurrence into a single tile, order preserved", () => {
    ...
    const out: Row[] = collapseGroupOccurrences(day as Row, gById);
  ...
  it("passes non-group rows through untouched", () => {
    const out: Row[] = collapseGroupOccurrences([{ id: "x", date: "1-Jun", time: "09:00", status: "scheduled" }] as Row, gById);
  ...
  it("keeps two different groups at different times separate", () => {
    ...
    const out: Row[] = collapseGroupOccurrences([
...
describe("group rows are accounting-invariant", () => {
  it("enrichPatientsWithBalance produces the same amountDue with or without group_id", () => {
    ...
    const withoutGroup = enrichPatientsWithBalance(patients, [{ ...base, id: "x1" }] as Row, now);
    const withGroup = enrichPatientsWithBalance(patients, [{ ...base, id: "x2", group_id: "g1" }] as Row, now);
