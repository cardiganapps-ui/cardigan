...
function daysAgo(n: number) { const d = new Date(NOW); d.setDate(d.getDate() - n); return iso(d); }
function hoursAgo(n: number) { const d = new Date(NOW); d.setHours(d.getHours() - n); return iso(d); }
...
describe("bucketLabel", () => {
  const t: Row = () => null; // force defaults
...
describe("groupNotesByRecency", () => {
  ...
  it("returns empty array for empty input", () => {
    ...
    expect(groupNotesByRecency(null as never, t, NOW)).toEqual([]);
  });
  ...
  it("pinned notes float to the top regardless of date", () => {
    ...
    expect(out[0].notes.map((n: Row) => n.id)).toEqual(["b"]);
  });
  ...
  it("preserves within-bucket input order", () => {
    ...
    const out = groupNotesByRecency(notes, t, NOW);
    expect(out[0].notes.map((n: Row) => n.id)).toEqual(["first", "second", "third"]);
  });
});
