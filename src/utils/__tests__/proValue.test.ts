...
function pad(n: number, value: (i: number) => Row, base: Row = {}) {
...
describe("computeProValue", () => {
  it("returns null when the user hasn't logged 10 sessions yet", () => {
    ...
    const result: Row = computeProValue(sessions, [], APR_30);
  ...
  it("sums payments in the current month and computes the Pro share %", () => {
    ...
    const result: Row = computeProValue(sessions, payments, APR_30);
  ...
  it("respects a custom session threshold for testing", () => {
    ...
    const result: Row = computeProValue(sessions, [], APR_30, { totalSessionThreshold: 1 });
    expect(result).not.toBeNull();
