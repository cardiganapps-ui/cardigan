...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- loosely-typed account rows
type Row = any;

function nameParts(fullName: string | null | undefined) {
  const tokens = (fullName || "").trim().toLowerCase().split(/\s+/).filter(Boolean);
  if (tokens.length === 0) return { first: "", last: "" };
  if (tokens.length === 1) return { first: tokens[0], last: "" };
  return { first: tokens[0], last: tokens.slice(1).join(" ") };
}
function compareNames(a: Row, b: Row) {
...
export function AdminUsers({ selectedId, onSelect, onClearSelection, onViewAs, currentAdminId }: {
  ...
  const [bulkPending, setBulkPending] = useState<string | null>(null); // "block" | "unblock" | "comp" | "uncomp" | "csv" | null
  ...
  useEffect(() => {
    ...
    return () => {
      el.removeEventListener("touchstart", onStart, opts as EventListenerOptions);
      el.removeEventListener("touchmove", onMove, opts as EventListenerOptions);
      el.removeEventListener("touchend", onEnd, opts as EventListenerOptions);
      el.removeEventListener("touchcancel", onEnd, opts as EventListenerOptions);
    };
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    let rows = accounts.filter((a: Row) => {
    ...
    if (sortKey?.key === "signup") {
      rows = rows.slice().sort((a: Row, b: Row) => ((a.firstSeen || "")).localeCompare(b.firstSeen || "") * dir);
    } else if (sortKey?.key === "patients") {
      rows = rows.slice().sort((a: Row, b: Row) => ((a.patientCount || 0) - (b.patientCount || 0)) * dir);
    } else {
      // Default: name asc; respect dir=desc by reversing.
      rows = rows.slice().sort((a: Row, b: Row) => compareNames(a, b) * dir);
    }
    return rows;
  ...
  const selectedAccounts = useMemo(() => {
    ...
    return accounts.filter((a: Row) => bulkSelected.has(a.userId));
  }, [accounts, bulkSelected]);

  const runBulk = useCallback(async (key: string, runOne: (a: Row) => unknown) => {
    if (bulkPending || bulkSelected.size === 0) return;
    setBulkPending(key);
    try {
      const results = await Promise.allSettled(selectedAccounts.map(runOne));
      const ok = results.filter((r: PromiseSettledResult<unknown>) => r.status === "fulfilled").length;
  ...
  const bulkActions = useMemo(() => [
    {
      ...
      label: "Otorgar comp",
      Icon: IconUserPlus,
      onClick: () => runBulk("comp", (a: Row) => adminGrantComp(a.userId, true)),
    },
    {
      key: "block",
      label: "Bloquear",
      Icon: IconShield,
      danger: true,
      onClick: () => runBulk("block", (a: Row) => adminBlockUser(a.userId, true)),
    },
    {
      key: "unblock",
      label: "Desbloquear",
      Icon: IconCheck,
      onClick: () => runBulk("unblock", (a: Row) => adminBlockUser(a.userId, false)),
    },
    {
      key: "csv",
      label: "Exportar CSV",
      ...
      onClick: () => {
        downloadCsv("cardigan-users-selected-{date}.csv", selectedAccounts, [
          { label: t("admin.userDetail.labelName"), get: (a: Row) => a.fullName || "" },
          { label: t("admin.userDetail.labelEmail"), get: (a: Row) => a.email || "" },
          { label: "Tier", get: (a: Row) => a.tier || "" },
          { label: t("admin.users.colPatients"), get: (a: Row) => a.patientCount },
          { label: t("admin.users.colSignup"), get: (a: Row) => a.firstSeen || "" },
          { label: "User ID", get: (a: Row) => a.userId },
        ]);
      },
    },
  ], [runBulk, selectedAccounts, t]);

  const selectionDisabled = useCallback((row: Row) => row.userId === currentAdminId, [currentAdminId]);

  const onExport = () => {
    downloadCsv("cardigan-users-{date}.csv", filtered, [
      { label: t("admin.userDetail.labelName"), get: (a: Row) => a.fullName || "" },
      { label: t("admin.userDetail.labelEmail"), get: (a: Row) => a.email || "" },
      { label: "Tipo", get: (a: Row) => a.accountType || "" },
      { label: t("admin.userDetail.labelProfession"), get: (a: Row) => a.profession || "" },
      { label: "Tier", get: (a: Row) => a.tier || "" },
      { label: t("admin.users.colPatients"), get: (a: Row) => a.patientCount },
      { label: "Bloqueado", get: (a: Row) => a.blocked ? "sí" : "no" },
      { label: t("admin.users.colSignup"), get: (a: Row) => a.firstSeen || "" },
      { label: "User ID", get: (a: Row) => a.userId },
    ]);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      width: 90,
      render: (a: Row) => (a.accountType === "patient" ? "—" : (a.patientCount ?? 0)),
    },
    {
      key: "signup",
      ...
      sortable: true,
      width: 110,
      render: (a: Row) => fmtDate(a.firstSeen),
    },
  ];

  const mobileLayout = (a: Row) => ({
  ...
  const handleSortByColumn = (next: Row) => {
    if (!next) {
        ...
        <div className="admin-split-view-list">
          ...
          {hasError ? (
            ...
            <AdminTable
              columns={columns}
              rows={filtered}
              rowKey={(a: Row) => a.userId}
              sort={sortKey}
              onSortChange={handleSortByColumn}
              onRowClick={(a: Row) => onSelect?.(a.userId)}
