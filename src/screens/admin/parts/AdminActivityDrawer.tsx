...
export function AdminActivityDrawer({ open, onClose, onJumpToUser }: {
  ...
  useEffect(() => {
    ...
    fetchAuditLog({ limit: 50 })
      .then((r: Row[]) => { if (!cancelled) { setRows(r || []); setLoaded(true); } })
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r: Row) => r.actor_id === actorFilter);
  }, [rows, actorFilter]);
    ...
    <>
      ...
      >
        ...
        <div className="admin-activity-drawer-toolbar">
          <select
            value={actorFilter}
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setActorFilter(e.target.value)}
