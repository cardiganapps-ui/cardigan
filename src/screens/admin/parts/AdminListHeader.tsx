...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- loosely-typed sort-config row
type Row = any;
      ...
      <div className="admin-list-header-v2-right">
        {sort && Array.isArray(sort.options) && (
          <select
            ...
            value={sort.value}
            onChange={(e: React.ChangeEvent<HTMLSelectElement>) => sort.onChange(e.target.value)}
            aria-label={sort.ariaLabel || "Ordenar"}
          >
            {sort.options.map((opt: Row) => (
