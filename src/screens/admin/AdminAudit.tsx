...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- loosely-typed audit-log rows
type Row = any;
...
export function AdminAudit() {
  ...
  const onCsv = () => {
    downloadCsv("cardigan-audit-{date}.csv", filtered, [
      { label: t("admin.audit.colDate"), get: (r: Row) => r.created_at },
      { label: t("admin.audit.colAction"), get: (r: Row) => r.action },
      { label: t("admin.audit.colActor"), get: (r: Row) => r.actor_id },
      { label: t("admin.audit.colTarget"), get: (r: Row) => r.target_user_id || "" },
      { label: t("admin.audit.colPayload"), get: (r: Row) => r.payload ? JSON.stringify(r.payload) : "" },
      { label: "IP", get: (r: Row) => r.ip || "" },
      { label: "User-Agent", get: (r: Row) => r.ua || "" },
    ]);
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (r: Row) => (r.actor_id ? `${r.actor_id.slice(0, 8)}…` : "—"),
    },
    {
      key: "target_user_id", label: t("admin.audit.colTarget"), mono: true, width: 110,
      render: (r: Row) => (r.target_user_id ? `${r.target_user_id.slice(0, 8)}…` : "—"),
    },
    {
