...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- loosely-typed user-detail bundle rows
type Row = any;
...
function fmtRelativeShort(iso: string | null | undefined) {
  if (!iso) return null;
...
function fmtMoneyCents(cents: number | null | undefined, currency = "MXN") {
                ...
                <>
                  ...
                  {invoices && invoices.length > 0 && (
                    <>
                      ...
                      <AdminTable
                        columns={[
                          { key: "created_at", label: t("admin.revenue.colDate"), render: (inv: Row) => fmtDate(inv.created_at), width: 130 },
                          { key: "amount_cents", label: t("admin.revenue.colAmount"), align: "right", width: 110,
                            render: (inv: Row) => fmtMoneyCents(inv.amount_cents, inv.currency || "MXN") },
                          { key: "paid_at", label: t("admin.revenue.colStatus"), width: 100,
                            render: (inv: Row) => inv.paid_at
                ...
                <AdminTable
                  columns={[
                    { key: "endpoint_host", label: "Endpoint host", mono: true,
                      render: (p: Row) =>p.endpoint_host || "—" },
                    { key: "created_at", label: "Registrado", width: 180,
                      render: (p: Row) =>fmtDateTime(p.created_at) },
                  ]}
                ...
                <AdminTable
                  columns={[
                    { key: "created_at", label: t("admin.audit.colDate"), width: 160,
                      render: (r: Row) =><span style={{ whiteSpace: "nowrap" }}>{fmtDateTime(r.created_at)}</span> },
                    { key: "action", label: t("admin.audit.colAction"),
                      render: (r: Row) =><span style={{ fontWeight: 600 }}>{auditLabel(r.action)}</span> },
...
function DetailContainer({ embedded, children }: { embedded?: boolean; children: React.ReactNode }) {
  if (embedded) {
...
function DefList({ rows }: { rows: Row[] }) {
  return (
    <dl className="admin-deflist">
      {rows.map(([label, value]: [React.ReactNode, unknown], i: number) => (
