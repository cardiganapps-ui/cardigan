...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- loosely-typed breadcrumb/section objects
type Row = any;
...
export function AdminLayout({ onViewAs, onLeaveAdmin, currentAdminId, onOpenPalette }: {
  ...
  const dragRef = useRef<{ mode: string; startX: number; startY: number; active: boolean } | null>(null);
  ...
  useEffect(() => {
    ...
    return () => {
      shell.removeEventListener("touchstart", onTouchStart, opts as EventListenerOptions);
      shell.removeEventListener("touchmove", onTouchMove, opts as EventListenerOptions);
      shell.removeEventListener("touchend", onTouchEnd, opts as EventListenerOptions);
      shell.removeEventListener("touchcancel", onTouchEnd, opts as EventListenerOptions);
    };
  }, []);

  const goSection = useCallback((section: string) => {
    route.navigate(section);
  }, [route]);

  const handleViewAs = useCallback((uid: string) => {
    onViewAs?.(uid);
  }, [onViewAs]);
    ...
    case "users":
      ...
      body = (
        <AdminUsers
          ...
          onSelect={(uid: string) => route.navigate("users", uid)}
  ...
  return (
    <div className="admin-shell" ref={shellRef}>
      ...
      <AdminActivityDrawer
        ...
        onJumpToUser={(uid: string) => route.navigate("users", uid)}
      />
    </div>
  );
}
