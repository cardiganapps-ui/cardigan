...
// eslint-disable-next-line @typescript-eslint/no-explicit-any -- loosely-typed invoice/overview rows
type Row = any;
...
export function AdminRevenue() {
  ...
  const onExport = () => {
    downloadCsv("cardigan-invoices-{date}.csv", invoices, [
      { label: t("admin.revenue.colDate"), get: (r: Row) => r.created_at || "" },
      { label: "User ID", get: (r: Row) => r.user_id },
      { label: t("admin.revenue.colAmount"), get: (r: Row) => ((r.amount_cents || 0) / 100).toFixed(2) },
      { label: "Moneda", get: (r: Row) => r.currency || "MXN" },
      { label: t("admin.revenue.colStatus"), get: (r: Row) => r.paid_at ? "Pagada" : "Pendiente" },
      { label: "Stripe URL", get: (r: Row) => r.hosted_invoice_url || "" },
    ]);
  };
  ...
  const columns = [
    {
      ...
      render: (r: Row) => <span style={{ whiteSpace: "nowrap" }}>{fmtDate(r.created_at)}</span>,
    },
    {
      key: "user_id", label: "User ID", mono: true, width: 110,
      render: (r: Row) => (r.user_id ? `${r.user_id.slice(0, 8)}…` : "—"),
    },
    {
      key: "amount_cents", label: t("admin.revenue.colAmount"), align: "right", width: 110,
      render: (r: Row) => fmtMoneyCents(r.amount_cents, r.currency || "MXN"),
    },
    {
      key: "paid_at", label: t("admin.revenue.colStatus"), width: 110,
      render: (r: Row) => r.paid_at
    ...
    },
    {
      ...
      render: (r: Row) => r.hosted_invoice_url ? (
