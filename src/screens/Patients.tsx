...
export function Patients() {
  ...
  // Edit form state
  // Edit-form state + the saveEdit branch machine (finalize / schedule-or-
  // rate change / basic info) live in usePatientEditForm — a money-write
  // path unit-tested directly (no e2e net). Sheet-mode flags (selected /
  // editing / confirmDelete) stay here and are passed in.
  const {
  ...
  const listJSX = (
    <PatientListView
      ...
      setPotentialProfile={setPotentialProfile}
      openPatientContextMenu={openPatientContextMenu}
        ...
        <div className="patients-detail-pane">
          {expediente ? (
            <PatientExpediente
              ...
              onEdit={(p: Row) => openEditForPatient(p)}
      ...
      {expediente && !isTabletSplit && createPortal(
        <PatientExpediente
          ...
          onEdit={(p: Row) => {
            ...
            setExpedienteOrigin(null);
            setHideFab?.(false);
            // Populate + open the edit sheet (shared with the other entry
            // points); the nav resets above are specific to this caller.
            openEditForPatient(p);
          }}
          createSession={createSession}
          createNote={createNote}
          updateNote={updateNote}
