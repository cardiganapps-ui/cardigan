...
function touchEvent(touches: Row[]): Row {
  return {
    touches: touches.map((t: Row) => ({ clientX: t.x, clientY: t.y })),
    cancelable: true,
...
describe("useLongPress", () => {
  it("fires onLongPress after the configured ms", () => {
    ...
    act(() => (result.current.bind as Row).onTouchStart(touchEvent([{ x: 10, y: 20 }])));
  ...
  it("does not fire when the user moves past tolerance", () => {
    ...
    const { result } = renderHook(() => useLongPress(cb, { ms: 300 }));
    act(() => (result.current.bind as Row).onTouchStart(touchEvent([{ x: 10, y: 20 }])));
    act(() => (result.current.bind as Row).onTouchMove(touchEvent([{ x: 30, y: 25 }])));
  ...
  it("tolerates small movement under the threshold", () => {
    ...
    const { result } = renderHook(() => useLongPress(cb, { ms: 300 }));
    act(() => (result.current.bind as Row).onTouchStart(touchEvent([{ x: 10, y: 20 }])));
    act(() => (result.current.bind as Row).onTouchMove(touchEvent([{ x: 13, y: 23 }])));
  ...
  it("cancels on early release before the ms elapses", () => {
    ...
    const { result } = renderHook(() => useLongPress(cb, { ms: 300 }));
    act(() => (result.current.bind as Row).onTouchStart(touchEvent([{ x: 10, y: 20 }])));
    act(() => (result.current.bind as Row).onTouchEnd(touchEvent([])));
    act(() => vi.advanceTimersByTime(500));
    expect(cb).not.toHaveBeenCalled();
  });
  ...
  it("preventDefault on touchend AFTER long-press fires (suppresses synthetic click)", () => {
    ...
    act(() => (result.current.bind as Row).onTouchEnd(endEvent));
  ...
  it("does nothing when onLongPress is null (parent opted out)", () => {
    ...
    expect(() => act(() => (result.current.bind as Row).onTouchStart(touchEvent([{ x: 0, y: 0 }])))).not.toThrow();
    act(() => vi.advanceTimersByTime(500));
  ...
  it("onClickCapture swallows the synthetic click that follows a fired long-press", () => {
    ...
    act(() => (result.current.bind as Row).onClickCapture(clickEvent));
  ...
  it("does not swallow keyboard activations long after a fired long-press", () => {
    ...
    act(() => (result.current.bind as Row).onClickCapture(earlyClick));
    ...
    act(() => (result.current.bind as Row).onClickCapture(lateClick));
