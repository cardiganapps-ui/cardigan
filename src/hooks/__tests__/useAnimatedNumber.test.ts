...
let rafCallbacks: Row[] = [];
let now = 0;

beforeEach(() => {
  rafCallbacks = [];
  now = 0;
  vi.stubGlobal("requestAnimationFrame", (cb: Row) => {
...
describe("useAnimatedNumber", () => {
  ...
  it("snaps to target when prefers-reduced-motion is set", () => {
    ...
    window.matchMedia = vi.fn(() => ({ matches: true, addEventListener: () => {}, removeEventListener: () => {} })) as Row;
    const { result } = renderHook(() => useAnimatedNumber(100, { duration: 700 }));
    act(() => {});
    expect(result.current).toBe(100);
    window.matchMedia = undefined as Row;
  });
  ...
  it("re-animates from current value when target changes mid-flight", () => {
    ...
    expect(result.current).toBeGreaterThan(partway as number);
