...
vi.mock("../../utils/heicConvert", () => ({ maybeConvertHeic: async (f: Row) => f }));
...
describe("softDeleteSession", () => {
  ...
  it("commit fires the supabase delete; no wire call before commit", async () => {
    ...
    const dels = mock.calls.filter((c: Row) => c.table === "sessions" && c.ops.some((o: Row) => o.op === "delete"));
...
describe("softDeletePayment", () => {
  ...
  it("commit fires the supabase delete", async () => {
    ...
    const dels = mock.calls.filter((c: Row) => c.table === "payments" && c.ops.some((o: Row) => o.op === "delete"));
    expect(dels).toHaveLength(1);
  });
});
...
describe("softDeleteExpense", () => {
  ...
  it("commit calls supabase delete + cascades the receipt document", async () => {
    ...
    const dels = mock.calls.filter((c: Row) => c.table === "expenses" && c.ops.some((o: Row) => o.op === "delete"));
    expect(dels).toHaveLength(1);
...
describe("softDeleteNote", () => {
  function seed() {
    ...
    const actions = createNoteActions(
      ...
      makeStateHolder(false), makeStateHolder(""), null as Row,
    );
  ...
  it("commit fires the supabase delete", async () => {
    ...
    const dels = mock.calls.filter((c: Row) => c.table === "notes" && c.ops.some((o: Row) => o.op === "delete"));
    expect(dels).toHaveLength(1);
  });

  it("temp-id soft delete: commit is local-only, no wire call", async () => {
    const notes = makeStateHolder([{ id: "temp-x", title: "Plan", content: "X" }]);
    const actions = createNoteActions("user-1", notes.get(), notes, makeStateHolder(false), makeStateHolder(""), null as Row);
