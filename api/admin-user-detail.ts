...
async function handler(req: Row, res: Row) {
  ...
  const sessions_30d = sessRows.filter((s: Row) => {
    const t = s.created_at ? new Date(s.created_at).getTime() : 0;
    return t > now - ms30d;
  }).length;
  const sessions_completed = sessRows.filter((s: Row) => s.status === "completed").length;
  const sessions_cancelled = sessRows.filter((s: Row) => s.status === "cancelled").length;
  const sessions_charged = sessRows.filter((s: Row) => s.status === "charged").length;

  const payRows = paymentStats.data || [];
  const payments_30d = payRows.filter((p: Row) => {
  ...
  const pushList = (pushSubs.data || []).map((p: Row) => {
    let host = null;
  ...
  const docsBytes = (docsStats.data || []).reduce((sum: number, d: Row) => sum + (d.file_size || 0), 0);
