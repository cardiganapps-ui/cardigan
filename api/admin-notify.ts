...
async function handler(req: Row, res: Row) {
  ...
  const row = (uid: Row) => ({ user_id: uid, kind: "system", title: cleanTitle, body: cleanBody, url: cleanUrl });
  ...
  if (broadcast) {
    ...
    for (;;) {
      ...
      const { error: insErr } = await svc.from("notifications").insert(users.map((u: Row) => row(u.id)));
