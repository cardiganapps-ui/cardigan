...
function validateCreatePayload(body: Row) {
...
async function handleList(req: Row, res: Row) {
  ...
  const ids = (codes || []).map((c: Row) => c.id);
  const usageByCode = new Map<Row, Row>();
  if (ids.length) {
...
async function handleToggle(req: Row, res: Row, admin: Row) {
  const body = typeof req.body === "string" ? safeJsonParse(req.body) : (req.body || {});
