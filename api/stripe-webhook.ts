...
export function shouldSkipStaleEvent(eventCreatedIso: Row, rowLastEventIso: Row) {
...
export function invoiceIsRewardEligible(invoice: Row) {
...
function isProState(status: Row, dpm: Row) {
  if (status === "active" || status === "past_due") return true;
  if (status === "trialing" && !!dpm) return true;
  return false;
}

async function applySubscriptionSnapshot(svc: Row, sub: Row, eventCreatedUnix: Row): Promise<Row> {
...
async function maybeCreditReferralReward(svc: Row, invoice: Row, customerId: Row): Promise<Row> {
...
async function reconcilePatientPaymentSuccess(svc: Row, pi: Row): Promise<Row> {
  const piId = pi.id;
...
async function reconcilePatientPaymentFailed(svc: Row, pi: Row, terminalStatus: Row): Promise<Row> {
  const piId = pi.id;
  if (!piId) return { ok: false, error: "no PI id" };
...
async function applyConnectAccountUpdate(svc: Row, account: Row, eventCreatedUnix: Row): Promise<Row> {
...
async function handleEvent(svc: Row, event: Row): Promise<Row> {
    ...
    case "checkout.session.completed": {
      ...
      try {
        ...
        const promoIds = (session?.discounts || [])
          .map((d: Row) => typeof d?.promotion_code === "string" ? d.promotion_code : d?.promotion_code?.id)
          .filter(Boolean);
...
async function handler(req: Row, res: Row) {
  ...
  const ok = secrets.some((secret: Row) =>
