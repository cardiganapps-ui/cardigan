...
function looksLikePII(text: Row) {
...
function validateMessages(messages: Row) {
...
async function handler(req: Row, res: Row) {
  ...
  const convo: Row[] = messages.slice();
  let totalUsage: Row = null;
  ...
  try {
    for (let turn = 0; turn < MAX_TOOL_TURNS; turn++) {
      ...
      const toolResults: Row[] = [];
      for (const block of final.content) {
        if (block.type !== "tool_use") continue;
        let resultPayload: Row;
        let isError = false;
        try {
