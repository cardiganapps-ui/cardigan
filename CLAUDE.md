...
- **`90062` "must contain a higher version" + `90186` "train is closed" — iOS marketing-version train closed (RECURRING; hit 20.5 on 2026-06-28 and 20.6 on 2026-07-08).** Apple locks a marketing version (`CFBundleShortVersionString`) once a build on it ships to the App Store, and rejects every later upload to that string — TestFlight included. Closed so far: **20.3, 20.4, 20.5, 20.6.** It *will* recur after each App Store release. **Fix (≈30 s, NOT an app-code change):** bump the marketing version to the next value — either set the repo **variable** `IOS_MARKETING_VERSION` (Settings → Secrets and variables → Actions → *Variables*; no code edit, doable from a phone) **or** edit `MARKETING_VERSION` in `scripts/apply-ios-config.sh` — then push. The `Upload to TestFlight` step now detects 90062/90186 and prints this exact remedy as a `::error::`, so a red build self-explains. Note: the build NUMBER (`CURRENT_PROJECT_VERSION` = `run_number`) already auto-increments every run — only the version STRING needs a per-release human bump.
