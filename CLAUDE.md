...
- **Cards & rows.** Reach for `.card` (white / radius-lg / `border-lt` / overflow-hidden) before inventing a wrapper. List rows use `.row-item` (12px gap, 13px v-pad / 16px h-pad, min-height 62, `--teal-mist` active state) — same in `.bal-row` for finance. KPI tiles are `.kpi-card`: tabular-nums, eyebrow over a 20–22px display number. Empty states center a 44×44 `--cream-dark` icon circle + 13px muted body.
