...
2. **Accounting uses raw DB rows + a date-aware predicate — never `enrichedSessions`.** The UI's "past scheduled → completed" auto-complete is a display affordance. Accounting must NOT read `enrichedSessions`; instead it iterates `upcomingSessions` (raw) and applies the predicate in `utils/accounting.ts::sessionCountsTowardBalance`. Any visual mark in the UI of "completed" MUST come out to the same answer the predicate produces, otherwise users see one number and owe another.
...
4. **Denormalized counters (`patient.billed`, `patient.paid`, `patient.sessions`) are now maintained by DB triggers** (`trg_payments_recalc_paid` from migration 068, `trg_sessions_recalc_counters` from migration 069). Hooks no longer issue follow-up patient counter UPDATEs — the trigger fires atomically with each session/payment write and runs the canonical predicate (`public.session_counts_at` in SQL) using the user's tz from `notification_preferences`. The SQL predicate **MUST stay in sync** with `utils/accounting.ts::sessionCountsTowardBalance` — if you change the JS predicate, update the SQL function in a migration AND mirror the change in `supabase/schema.sql`'s trigger block. The audit script reconciles both predicates against the same data nightly; drift fails CI. `recalcPatientCounters` (`utils/patients.ts`) stays as a manual recovery tool, no longer plumbed into the hot path.
...
8. **Auto-extend derives the schedule from FUTURE sessions only.** `computeAutoExtendRows` in `utils/recurrence.ts` filters `scheduledRegular` to rows whose `date >= today`. Past `status='scheduled'` rows must NEVER feed the schedMap — they're auto-display "completed" but stay scheduled in the DB (per rule above), and including them regenerates phantom future sessions on slots the user has already moved away from. Tests in `utils/__tests__/recurrence.test.ts` lock this in; the regression test deliberately uses `status=SCHEDULED` for past abandoned slots because `status=COMPLETED` (the earlier test variant) silently passed despite the bug shipping. If you need to walk historical rows for any reason, do it OUTSIDE the schedule-derivation path.

If you are about to touch anything in `useCardiganData`, `usePatients`, `useSessions`, `usePayments`, `utils/patients.ts`, `utils/recurrence.ts`, `supabase/schema.sql`, or the `sessions` / `payments` / `patients` tables — re-read this section first.
...
- **PWA:** `vite-plugin-pwa` with `injectManifest` strategy, custom `src/sw.ts`
...
npm run test -- dates    # Run a single test file (matches utils/__tests__/dates.test.ts)
...
`src/hooks/useCardiganData.ts` is the coordinator. It owns network fetches and composes 5 domain action modules (`usePatients`, `useSessions`, `usePayments`, `useNotes`, `useDocuments`). On load it:
...
Mutations go through the domain hooks, which update Supabase and local state optimistically. The result is injected into `CardiganContext` (`src/context/CardiganContext.tsx`) and consumed via `useCardigan()`.
...
- **Dates are stored as `"D-MMM"` strings** (Spanish months: `"8-Abr"`) in `sessions.date` and `payments.date`. Parsers accept the legacy space-separated form too, and `useCardiganData::mapRows` normalizes on read so the UI never sees the old format. Convert with `utils/dates.ts` (`formatShortDate`, `shortDateToISO`, `isoToShortDate`; `formatShortDateWithYear`/`isoToShortDateWithYear` for the rare case where year context matters, rendered as `"8-Abr-26"`). Date inputs use ISO; display uses short form.
- **Auto-complete is display-only.** Past `scheduled` sessions render as `completed` but are NOT persisted. Users can override any session's status to any other (including reverting to scheduled). See `SESSION_STATUS` in `data/constants.ts` — the DB check constraint mirrors this and must stay in sync.
- **Tutor sessions** (for minor patients) are marked by a `"T·"` prefix on `sessions.initials`. Helpers in `utils/sessions.ts`. Purple styling is derived from this prefix.
...
- **Expenses (Gastos) — money-out ledger.** Expenses are pure overhead (no `patient_id`). Recurring templates auto-generate one expense per (template, year, month) slot, deduplicated by the partial unique index `uniq_expenses_recurring_period`. Any code path that inserts a recurring-generated row MUST handle 23505 cleanly (skip / no-op, never crash) — same invariant as `uniq_sessions_patient_date_time`. Auto-backfill is **capped at `RECURRING_EXPENSE_AUTO_BACKFILL_MONTHS`** (currently 2) — anything older surfaces as a one-tap "Generar N gastos pendientes" prompt on the Gastos tab. **Silently inserting beyond that cap is a prime-directive violation.** Receipts: deletion of an expense MUST first call `deleteDocument(receipt_document_id)` (in `useExpenses::deleteExpense`) to clean up the document row + R2 object; the DB-side `on delete set null` is a backstop only. Personal-treatment expenses (`tax_treatment='personal'`) are excluded from the P&L view but kept in the ledger so therapists can use one app for everything. The `expenses.category` check constraint mirrors `EXPENSE_CATEGORIES` in `data/constants.ts` — keep them in sync.

### Database & security
- `supabase/schema.sql` is the canonical schema; forward-looking incremental changes go in numbered files under `supabase/migrations/`. Already-applied catch-up migrations live in `supabase/migrations/archive/` (kept for history, don't re-run). Keep the `sessions.status` / `payments.method` / `expenses.category` / `expenses.tax_treatment` / `documents.kind` check constraints in sync with `SESSION_STATUS` / `PAYMENT_METHODS` / `EXPENSE_CATEGORIES` / `TAX_TREATMENTS` (`data/constants.ts`), and keep `ADMIN_EMAIL` in sync with the `is_admin()` function in `schema.sql`.
- Every table has RLS `auth.uid() = user_id`. Admin read-all policies use the `is_admin()` SQL helper (checks JWT email).
- Service-role key is ONLY used in `api/` (Vercel serverless) via `api/_admin.ts::getServiceClient()`. Admin endpoints must call `requireAdmin(req, res)` first. Never reference `SUPABASE_SERVICE_ROLE_KEY` from anything under `src/`.

### Serverless API (`api/`)
- `_admin.ts`, `_r2.ts`, `_push.ts` are shared helpers — they must verify the caller's JWT before using the service-role client.
- `upload-url.ts` / `document-url.ts` / `delete-document.ts` issue presigned R2 URLs; `_r2.ts::validatePath` enforces `${userId}/…` prefix and blocks traversal.
- `send-session-reminders.ts` is the web-push cron (auth'd by `CRON_SECRET`). `push-subscribe.ts` / `push-unsubscribe.ts` manage `push_subscriptions`. See `supabase/migrations/006_push_notifications.sql` and `007_push_cron.sql`.
- `admin-block-user.ts` / `admin-delete-user.ts` are admin-only mutations over `auth.users`.

### Service worker & updates
`main.tsx` registers `/sw.js` with `updateViaCache: 'none'`, polls for updates on focus and every 30 min, and dispatches `cardigan-update-ready` events. `components/UpdatePrompt.tsx` surfaces the "Actualización disponible" toast; tapping it posts `SKIP_WAITING` and reloads on `controllerchange`. Do NOT auto-activate waiting SWs — it would reload mid-action.

### Screens & layering
- Routing is hash-based (`useNavigation`). App shell in `App.tsx` renders one screen at a time; overlays (sheets, modals, viewers) stack via `useLayer` which wires Escape/back-button dismissal.
...
- **Client opt-in:** `src/supabaseClient.ts` sets `auth.experimental.passkey: true` (needs supabase-js ≥ 2.105). The UI is double-gated in `src/config/passkeys.ts`: the build flag **`VITE_PASSKEYS_UI_ENABLED`** (set true on Vercel Prod+Preview) AND `passkeysSupported()` (web: any `PublicKeyCredential` browser; native: **iOS only** — Android's clientDataJSON origin is the app signature, which Supabase rejects).
...
Apple and Google are **additional** login options wired through `useAuth.signInWithProvider(provider)` and rendered by `AuthForm` (both the Entrar and Crear cuenta tabs). `App.tsx` must pass `onProvider={signInWithProvider}` to `AuthScreen` or no provider buttons render (this was missing originally).
- **Apple** — `external_apple_enabled=true`. iOS native uses `@capawesome/capacitor-apple-sign-in` → `signInWithIdToken` (`src/lib/nativeAppleSignIn.ts`, nonce hashed per Apple's scheme); web/Android use the OAuth redirect. Always on.
- **Google** — `external_google_enabled=true`, gated in the UI behind **`VITE_GOOGLE_OAUTH_ENABLED`** (Vercel Prod+Preview). Web uses `signInWithOAuth` (code exchange). iOS native uses `@capgo/capacitor-social-login`'s Google picker → `signInWithIdToken` (`src/lib/nativeGoogleSignIn.ts`). The native ID token's `aud` is the **iOS** OAuth client ID, so Supabase's `external_google_client_id` holds BOTH the web and iOS client IDs (comma-joined) as valid audiences. Native runs **no nonce** + `external_google_skip_nonce_check=true` (propagating a nonce through the native SDK is fragile; the iOS client ID is bundle-bound so replay risk is low; web is unaffected — it uses code exchange). The reversed iOS client-ID **URL scheme** is injected into `Info.plist` `CFBundleURLTypes` by `scripts/apply-ios-config.sh` (must survive CI regen). Google Cloud project `17610829726`; OAuth clients are Web (Supabase provider + audience) + iOS (native) + Android (native, verification-only). Client IDs are public; only the **web client secret** is secret (lives in Supabase config, never in the repo). **Android native Google IS wired** (same `nativeGoogleSignIn.ts`, Credential Manager sheet): the plugin uses `webClientId` as the server client ID, so the Android ID token's `aud` is the WEB client ID — already an accepted audience, no Supabase change. It only works once an **Android-type OAuth client** (package `mx.cardigan.app` + the Play App Signing SHA-1, plus the upload/debug SHA-1 for local builds) exists in the Google Cloud project — Credential Manager uses it purely to verify the caller; no ID from it ships in the app. Until then the picker rejects with a visible error. **Apple is hidden on Android native** (`AuthScreen.tsx` gates it) — no Apple SDK there and the OAuth redirect can't round-trip to `capacitor://localhost`.

### Vercel serverless routes (`api/`)
Files under `api/*.ts` become `/api/*` routes — but **files with names starting with `_` or `__` are NOT exposed as routes** (which is why `_admin.ts` / `_push.ts` / `_r2.ts` / `_sentry.ts` work as helpers). Diagnostic endpoints need a plain name like `cron-debug.ts` to be reachable.
...
- **Opt-in is per-patient.** `patients.whatsapp_enabled` + `patients.whatsapp_consent_at`. Toggle in NewPatientSheet (creation) and Patients.tsx (edit) — disabled until a phone is set. The consent timestamp is the LFPDPPP audit record.
- **Recipient is `patients.phone`** in every case. For a minor patient the phone is already the tutor's (per product direction); we don't keep a separate parent-phone column. Greeting variable is `parent` if present, otherwise `name`. Stored as digits-only via `phoneDigits()`; normalized to E.164 at send time via `api/_whatsapp.ts::toE164MX()` (prepends `+52` for 10-digit MX numbers).
...
- **Webhook:** `api/whatsapp-webhook.ts`. GET handles Meta's subscribe handshake (echoes `hub.challenge` when `hub.verify_token` matches `WHATSAPP_WEBHOOK_VERIFY_TOKEN`). POST verifies HMAC-SHA256 over the raw body using `WHATSAPP_APP_SECRET` against the `X-Hub-Signature-256` header (mirror of `api/resend-webhook.ts`). Disable Vercel's body parser via `export const config = { api: { bodyParser: false } }`.
...
- **Privacy:** `src/data/privacy.ts` enumerates Meta Platforms Ireland as a transferee and lists the fields shared per send. Bumped `POLICY_VERSION` so existing users re-accept via `ConsentBanner`.
- **UI is gated** behind `VITE_WHATSAPP_UI_ENABLED` (build-time Vite env var). Default = unset → patient toggles in `NewPatientSheet` and `Patients.tsx` are not rendered. The DB columns, server endpoints, and cron branch are all live but inert because no patient can opt in via UI. Flip `VITE_WHATSAPP_UI_ENABLED=true` in Vercel Production + Preview and redeploy when the Meta template + env vars are ready.
- **Privacy** copy: while gated, `src/data/privacy.ts` does NOT mention Meta or WhatsApp — adding the paragraph back will require bumping `POLICY_VERSION` to force re-acceptance, so do that in the same commit that flips the UI gate on.
...
- **Generator:** `api/_calendar.ts::generateICS()` is the pure helper; tests in `api/__tests__/calendar.test.ts` lock down RFC 5545 envelope shape, line folding, escape rules, DTSTART/DTEND TZID anchoring, and that `SUMMARY` includes the full patient name (with initials as a fallback when the name is missing).
...
- **Threat model:** a Supabase DB compromise alone should NOT yield therapy-note plaintext. Crypto code lives in `src/lib/cryptoNotes.ts`; tests in `src/utils/__tests__/cryptoNotes.test.ts`.
- **Master key** (32 bytes, AES-256) is generated client-side and never leaves the browser in plaintext. Two wraps are stored on the server in `user_encryption_keys` (migration 017):
  ...
  - `recovery_wrap` — RSA-OAEP-2048 under the public key bundled in the client bundle (`VITE_NOTES_RECOVERY_PUBLIC_KEY`). The matching private key (`NOTES_RECOVERY_PRIVATE_KEY`) lives only in the server env and is read solely by `api/admin-recover-encryption.ts`.
...
- **Policy version** lives in `src/data/privacy.ts::POLICY_VERSION`. When the policy body changes materially, bump the version string; users whose latest accepted version no longer matches are re-prompted on next login via `components/ConsentBanner.tsx`.
...
- **ARCO flows** (Acceso, Rectificación, Cancelación, Oposición) are wired through three endpoints:
  ...
  - `POST /api/delete-my-account` — cascade-deletes the user (via `api/_admin.ts::deleteUserCascade`, shared with `admin-delete-user.ts` so both flows can't drift). Requires `confirmation: "ELIMINAR"` in the body.
...
- **Hook + UI:** `src/hooks/useSubscription.ts` exposes `accessState` (`loading | trial | active | expired`), `daysLeftInTrial`, and helpers `startCheckout({ referralCode })` / `openPortal()` / `fetchReferralInfo()`. Trial-expired and trial-ending banners live in `App.tsx`; the full panel is `Settings → Suscripción` (`activeSheet === "plan"`).
...
- **Rotating a noisy error:** silence it in the Sentry UI (create an inbound filter or ignore rule) rather than adding a try/catch that hides a real failure. If you need to suppress at the code level, add an allowlist check inside the specific handler — do NOT globally suppress in `_sentry.ts`.
...
- **Skew Protection:** `skewProtectionMaxAge: 43200` (12h). Old API surface stays alive 12h after a new deploy so a stale-tab mutation doesn't blow up. Client opts in via `src/lib/skewProtection.ts` stamping `x-deployment-id` on every same-origin `/api/*` fetch.
...
**Triggers:** push to `main` touching app code (the workflow ignores `**.md`, `android/**`, `scripts/**` except `apply-ios-config.sh`, and `.github/**` except itself), or a manual `workflow_dispatch` on `ios-build.yml`. The build number is `github.run_number` (monotonic — Apple requires each upload's `CURRENT_PROJECT_VERSION` to strictly increase). The **marketing version** (`CFBundleShortVersionString`) is derived automatically at build time from App Store Connect (`scripts/resolve-ios-version.mjs`) — no manual bump needed; details under "build-failure triage" below.
...
**Diagnosing a "failed" run — build-failure triage (iOS + Android).** First pull the logs (`mcp__github__get_job_logs` with `failed_only: true`, `return_content: true`) and match the symptom below. A build that **archives + exports and then fails at `Upload to TestFlight`** is almost never a code bug — it's one of the version/quota rejections here. These recur, so treat them as known-issues, not new investigations:

- **`90062` "must contain a higher version" + `90186` "train is closed" — iOS marketing-version train closed (historically RECURRING; hit 20.5 on 2026-06-28 and 20.6 on 2026-07-08 — now AUTOMATED away).** Apple locks a marketing version (`CFBundleShortVersionString`) once a build on it ships to the App Store, and rejects every later upload to that string — TestFlight included. Closed so far: **20.3, 20.4, 20.5, 20.6.** **This no longer needs a manual bump:** the iOS workflow's "Resolve iOS marketing version" step (`scripts/resolve-ios-version.mjs`) queries App Store Connect at build time and auto-picks a version strictly above the highest released one (reusing the current open draft when there is one), so it advances on its own after each release. It fails **open** — if ASC is unreachable it falls back to the `MARKETING_VERSION` default in `apply-ios-config.sh` and the build still runs. If you ever DO see 90062/90186 (the rare ASC-down-at-build-time case), the `Upload to TestFlight` step prints the exact remedy as a `::error::`; the fix is to set the repo **variable** `IOS_MARKETING_VERSION` (Settings → Secrets and variables → Actions → *Variables*; takes precedence over the resolver, no code edit, doable from a phone). The build NUMBER (`CURRENT_PROJECT_VERSION` = `run_number`) auto-increments every run as before.
- **`90382` "Upload limit reached" — Apple's ~24 h daily upload cap.** Not our code; the IPA built + signed fine. Wait for the window to reset, then re-trigger. (This is why you batch pushes to `main` — see the ⚠️ note above.)
- **Android `setup-android` "Error on ZipFile unknown archive" (RECURRING, transient infra).** A corrupted Android SDK download on the runner — the job dies in ~30 s, before `npm ci`. Not our code. Just re-trigger.

**Re-triggering a build when the re-run API is blocked.** In an agent session BOTH the GitHub MCP integration and the sandbox HTTPS proxy **cannot re-run or dispatch workflows** (403 "Resource not accessible by integration" / "GitHub access is not enabled for this session") — the `.env.local` `GITHUB_TOKEN` is a proxy-injected placeholder (`proxy-injected`, 14 chars), NOT a usable PAT, so REST calls to `rerun-failed-jobs` / `dispatches` fail. Git **push** works (the proxy injects git credentials), so re-trigger by pushing a commit that touches the target workflow's watched paths: for iOS push a one-line touch to `scripts/apply-ios-config.sh`, for Android touch `scripts/apply-android-config.sh` — each is the *other* platform's ignored path, so it fires exactly ONE build. A no-op empty commit does NOT trigger anything (`paths-ignore` needs a changed file). To land a CI / docs change WITHOUT firing any build, put `[skip ci]` in the commit message. From a browser/phone, the workflow's **Run workflow** button (`workflow_dispatch`) also works.
...
- **The FIRST upload must be manual.** Google won't let the API create the very first release of a new app — upload one AAB by hand in the Play Console (and enroll in Play App Signing) before CI's `Upload to Play` step can succeed. After that, every push to `main` ships automatically. The build uploads the signed AAB as a downloadable artifact (`cardigan-release-aab`) so this first upload is doable from a phone (the Play upload step failing on run #1 is expected, not a code bug).
- **Monetization matches iOS on native (reader-app on both platforms).** The gate is `isNative()` (NOT `isNative() && isIOS()`) in `ProUpgradeSheet.tsx`, `AppBanners.tsx`, and `PlanSheet.tsx` — no price/subscribe CTA/external link inside either native app. Native shows inert hints (`pro.nativeHint`, `subscription.expiredBannerNative`, `subscription.nativeReaderHint`); web keeps the full Stripe checkout. Pro is sold on the web; existing subscribers just sign in. Don't reintroduce an in-app buy surface on native without integrating the platform's billing (Apple IAP / Google Play Billing).

**Phone-only keystore:** `android-keystore-bootstrap.yml` (workflow_dispatch, type `generate` to confirm) mints the upload keystore in CI and hands back the base64 + passwords as a 1-day artifact — for setting up from an iPhone/Android with no `keytool`. Delete the artifact after copying values into secrets. The equivalent local path is `scripts/android-keystore.sh`. Full phone walkthrough in `docs/play-store-submission.md`.
...
- **Loading states.** Long-running fetches paint a skeleton that mirrors the eventual layout (cards in the right slots, with `.sk-bar` / `.sk-circle` shimmer rectangles standing in for content). See `App.tsx::LoadingSkeleton` for the therapist Home + Finances variants and `PatientHome.tsx::PatientHomeSkeleton` for the patient hero+balance+therapist-card layout. The shimmer animation is paused under `prefers-reduced-motion` automatically. **Don't ship a bare "Cargando…" string** — first paint should always feel like the destination.
...
- **Safe areas.** ALWAYS consume the tokens — `--sat` (top), `--sab` (bottom), `--sal`/`--sar` (sides) — and NEVER raw `env(safe-area-inset-*)`. The tokens are dual-sourced in base.css: they prefer the `--safe-area-inset-*` vars Capacitor 8's SystemBars plugin injects on Android (where `env()` resolves to 0 on most WebViews) and fall back to `env()` for iOS/web. A raw `env()` consumer silently gets 0 on Android native. Sticky top bars use `padding-top: calc(var(--sat) + 14px)`. Sheets get `padding-bottom: var(--sab)`. Scroll containers reserve home-indicator clearance via `padding-bottom: max(16px, var(--sab))`. The therapist `.shell` + `.main-content` split is the reference: outer fixed `100dvh` + `overflow: clip`, inner `flex: 1` + `overflow-y: auto`. Patient surfaces should mirror it — body has `overflow: hidden` globally, so every full-viewport screen MUST own its own scroll container.
...
- **Liquid Glass is tokenized — never hand-roll a frosted surface.** All chrome glass (floating topbar wrapper `.app-chrome-top`, tab pill, FAB, toasts, banners, ⌘K, slash menu) consumes the `--glass-*` tokens in `base.css` (bg/filter/edge/shadow in three grades: `chrome`/`bar`/`panel`, + `--glass-tint-accent` for the FAB) or the `.glass-chrome/.glass-bar/.glass-panel/.glass-tinted` utility classes in `components.css`. Consuming the tokens buys you, for free: the dark-mode flip, the `@supports` no-backdrop-filter solid fallback, `prefers-reduced-transparency: reduce` (solid chrome), and `prefers-contrast: more` (hardened edges) — all central token swaps. Rules: (1) **never stack two backdrop-filter surfaces** — WebKit composites them out of z-order (that's why sheets hide `.bottom-tabs`/`.fab` via `:has()`, and why children of `.app-chrome-top` drop their own blur); (2) glass is for the floating chrome layer ONLY — content cards and sheet panels stay opaque; (3) `.page` reserves the floating chrome's height via `--chrome-top-overlap` (measured into `--chrome-top-h` by App.tsx) — mirror that pattern for any new floating chrome. True refraction: Chromium gets SVG-displacement refraction via `src/lib/glassRefraction.ts` (`html.glass-refract`); native iOS 26+ replaces the DOM tab pill with a real SwiftUI `glassEffect` bar (`plugins/native-chrome` + `src/lib/nativeChrome.ts`, `html.native-chrome` bumps `--bottom-tabs-h` to 80px for the webview's 0.8 zoom); Safari/iOS web keeps the hand-tuned CSS glass.
...
- When adding or changing a session status / payment method / patient lifecycle value, update `data/constants.ts` AND the DB check constraint in `supabase/schema.sql` (plus a migration).
- **Profession enum** (`PROFESSION` in `data/constants.ts`) is mirrored in three other places that must stay in sync: the `user_profiles.profession` check constraint in `supabase/schema.sql` / migration `021_user_profiles.sql`, the `ALLOWED` set in `api/admin-update-profession.ts`, and the keys in `src/i18n/vocabulary.ts`. Adding a profession requires touching all four. Profession is locked at sign-up (chosen via `src/screens/ProfessionOnboarding.tsx`); only admin can change it afterwards via `AdminPanel`. The active profession flows through `CardiganContext` (`useCardigan().profession`) and is also pushed into `I18nProvider` so `t("…{client.s}…")`-style placeholders resolve to the right vocabulary (`src/i18n/vocabulary.ts`).
